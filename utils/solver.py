...
#                   'rel': []}
    trace_dict = {'abs': [(F[:,0] - X[:,0]).view_as(x0).norm().item(),(F[:,1] - X[:,1]).view_as(x0).norm().item()],
                  'rel': [(F[:,0] - X[:,0]).view_as(x0).norm().item()/(1e-5 + F[:,0].norm().item()),(F[:,1] - X[:,1]).view_as(x0).norm().item()/(1e-5 + F[:,1].norm().item())]}
    lowest_dict = {'abs': 1e8,
                   'rel': 1e8}
    lowest_step_dict = {'abs': 0,
                        'rel': 0}
    ...
    for k in range(2, threshold):
        ...
        # alpha = torch.linalg.solve(H[:, :n+1, :n+1], y[:, :n+1])[:, 1:n+1, 0]
        alpha = torch.linalg.solve(H[:, :n+1, :n+1], y[:, :n+1])[:, 1:n+1].squeeze(-1)  # (bsz x n)
        
        X[:,k%m] = beta * (alpha[:,None] @ F[:,:n])[:,0] + (1-beta)*(alpha[:,None] @ X[:,:n])[:,0]
        # X[:, k % m] = beta * torch.matmul(alpha[:, None], F[:, :n]).squeeze(1) + (1 - beta) * torch.matmul(alpha[:, None], X[:, :n]).squeeze(1)
